...
Core::Core(): if_of(pipeline), of_ex(pipeline), ex_ma(pipeline), ma_rw(pipeline), PC(true){
	isProcidBranch = false;
	printVar = 2;
	initCore = false;
}

void Core::setCoreId(const unsigned id){
	coreId = id;
}
// void Core::writePC(const unsigned pc){
// 	//std::cout << "writePC " << pc<<'\n';
// 	PC.Write(pc);
// }

// void Core::setDestructionPC(const unsigned pc){
// 	destructionPC = pc;
// }

unsigned Core::getDestructionPC(){
	return destructionPC;
}

void Core::write_memory() {
	//std::cout << "in Core write_memory" << '\n';
	if (specifymemout){
		ofstream data_file;
...
void Core::fetch_begin() {
	...
	if (checkValidPC(PC.Read())){
		...
		fprint(printVar)<<" PC=0x"<<hex<<temp_PC<<"; "<<disassemble(temp_instruction_word);
...
void Core::fetch_end() {
	...
	if(isWaitInst){
		if(isBranchWait){
			fprint(printVar)<<" "<<"Waiting";
		}
		else {
			fprint(printVar)<<" "<<"Event Received";
		}
	}
	fprint(printVar)<<std::endl;
	if((R[0] == coreId) && isProcidBranch){
		initCore = true;
		printVar = 1;
		destructionPC = temp_PC + 8;
	}
	if(isProcidBranch && (!initCore)){
		PC.Write(temp_PC + 8);
	}
		...
		//std::cout << "Core fetch_end " << branchPC<< '\n';
		PC.Write(branchPC);
	}
...
void Core::decode() {
	...
	if (temp_isSt){
		...
		if (!temp_isImmediate){
			fprint(printVar)<<"; "<<registerstring(temp_rs2)<<" = 0x";
		}
	}
	if (!temp_isImmediate){
		fprint(printVar)<<hex<<temp_operand2;
	}
	else{
		fprint(printVar)<<"; imm = 0x"<<hex<<temp_immx;
	}
...
void Core::execute() {
	...
	if (temp_isRet){
		//std::cout << getDestructionPC() << '\n';
		if(temp_A == getDestructionPC()){
			initCore = false;
			printVar = 2;
			branchPC = multiPC;
			R[0] = -1;
	...
	if( temp_isWFE ){
		...
		if(chk)	{
			...
			for(unsigned i =0; i<NUM_CORES;i++){
				sendEvent[coreId][i] = true;
			}
		}
		else	isBranchWait = true;
  }else{
    isBranchWait = false;
		isWaitInst = false;
  }
  if( temp_isSEV ){
		for(unsigned i = 0, tmp = 1;i<NUM_CORES;i++, tmp *= 2){
			if((temp_B & tmp)!= 0){
				sendEvent[i][coreId] = false;
				//std::cout << temp_B<<" "<<i<<" "<<tmp << '\n';
			}
		}
  }
	//=======================================================================================
...
//TODO: need to add WFE and sendEvent
...
void Core::write_back() {
	...
	if (temp_isWb){
		...
		fprint(printVar)<<"; "<<registerstring(temp_addr)<<" = 0x"<<hex<<temp_result;
...
void Core::decTobinary(std::string &inst, unsigned imm){
	//std::stringstream ss;
	int tmp = 0;
	//bool chk = false;
