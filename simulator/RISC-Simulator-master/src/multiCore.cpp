...
Memory MEM(MEM_CAPACITY);
int multiPC = 0;
//bool sendEvent[NUM_CORES][NUM_CORES];

multiCore::multiCore(): if_of(pipeline), of_ex(pipeline), ex_ma(pipeline), ma_rw(pipeline), PC(true) {
  ...
  coreObjs = new Core[NUM_CORES-1];
  for(unsigned int i = 0; i<NUM_CORES-1;i++) {
    coreObjs[i].setCoreId(i);
  }

  for(unsigned k = 0;k<NUM_CORES;k++){
    for (size_t p = 0; p <NUM_CORES; p++) {
      sendEvent[k][p] = true;
    }
  }
...
void multiCore::load_program_memory(){
...
void multiCore::write_memory() {
  //std::cout << "in multiCore write_memory" << '\n';
	if (specifymemout){
		ofstream data_file;
...
void multiCore::reset_proc(){
  ...
  for(unsigned k=0;k<NUM_CORES-1;k++){
    coreObjs[k].reset_proc();
  }

}

void multiCore::run_simplesim(){
    ...
    //std::cout << std::hex<<"multi PC" <<PC.Read()<< '\n';
		fetch_begin();
		decode();
		execute();
		mem_access();
    ...
    //std::cout << NUM_CORES << '\n';
    for(unsigned int cores = 0;cores<NUM_CORES-1;cores++){
        coreObjs[cores].run_simplesim();
    }
...
void multiCore::fetch_end() {
	//std::cout << std::boolalpha<<isWaitInst << '\n';
	if(isWaitInst){
		if(isBranchWait)	{
			fprint(1)<<" "<<"Waiting";
		}
		else {
			fprint(1)<<" "<<"Event Received";
		}
	}
	fprint(1)<<endl;
	unsigned int temp_PC = PC.Read();
  if(isProcidBranch){
    PC.Write(temp_PC + 8);
    //initCoreid = R[0];
    //coreObjs[R[0]].setInitCore();
    //coreObjs[R[0]].writePC(temp_PC + 4);
    //std::cout<<"R[0] = "<<R[0]<<" "<<temp_PC<<std::endl;
    //coreObjs[R[0]].setDestructionPC(temp_PC + 8);
  }
  else if(isBranchWait){
    PC.Write(temp_PC);
  }
...
void multiCore::decode() {
  ...
  //========================================================================================
  of_ex.isProcid.Write(temp_isProcid);
  of_ex.isWFE.Write(temp_isWFE);
  of_ex.isSEV.Write(temp_isSEV);
  //========================================================================================
...
void multiCore::execute() {
  ...
  //===================================================================================
  bool temp_isProcid = of_ex.isProcid.Read();
  bool temp_isWFE = of_ex.isWFE.Read();
  bool temp_isSEV = of_ex.isSEV.Read();
  //===================================================================================
	//////////   Branch Unit  ///////////
  ...
  if( temp_isProcid ){
    //std::cout << "temp_isProcid" << '\n';
    isProcidBranch = true;
    temp_aluResult = temp_B;

  }else{
    isProcidBranch = false;
  }
  ...
  if( temp_isWFE ){
		...
		isWaitInst = true;
		//std::cout << temp_B << '\n';
		for(unsigned i = 0, tmp = 1; i<NUM_CORES; i++, tmp *= 2){
			if((temp_B & tmp) != 0){
				//std::cout<<std::boolalpha << "Hello "<<i <<sendEvent[NUM_CORES-1][i] <<'\n';
				if(sendEvent[NUM_CORES-1][i]){
					chk = false;
					break;
				}
			}
		...
		if(chk)	{
			//std::cout << "chk is true | wrong | " <<std::boolalpha<<isWaitInst<< '\n';
			isBranchWait = false;
			for(unsigned i =0; i<NUM_CORES;i++){
				sendEvent[NUM_CORES-1][i] = true;
			}
		}else{
			//std::cout << "isBranchWait = true" << '\n';
			//std::cout << "isWaitInst = false" << '\n';
			isBranchWait = true;
		}
  }else{
		//std::cout << "In else of temp_isWFE" << '\n';
    isBranchWait = false;
		isWaitInst = false;
  }
  if( temp_isSEV ){
  ...
  //==================================================================================
  ex_ma.isProcid.Write(temp_isProcid);
  ex_ma.isWFE.Write(temp_isWFE);
  ex_ma.isSEV.Write(temp_isSEV);
  //==================================================================================
...
void multiCore::mem_access() {
  ...
  //==================================================================================
  bool temp_isProcid = ex_ma.isProcid.Read();
  bool temp_isWFE = ex_ma.isWFE.Read();
  bool temp_isSEV = ex_ma.isSEV.Read();
  //==================================================================================
  ...
  //=========================================================================================
  ma_rw.isProcid.Write(temp_isProcid);
  ma_rw.isWFE.Write(temp_isWFE);
  ma_rw.isSEV.Write(temp_isSEV);
  //=========================================================================================
...
void multiCore::write_back() {
  ...
  //====================================================================================
  bool temp_isProcid = ma_rw.isProcid.Read();
  bool temp_isWFE = ma_rw.isWFE.Read();
  bool temp_isSEV = ma_rw.isSEV.Read();
  //====================================================================================
...
bool multiCore::checkValidPC(unsigned int testPC){
...
unsigned int multiCore::inst_bitset(unsigned int inst_word, unsigned int start, unsigned int end){
	inst_word = inst_word<<(32-end);
	inst_word = inst_word>>(32-end + start-1);
	return inst_word;
}

bool multiCore::check_data_conflict(PipelineRegister& A, PipelineRegister& B){
...
bool multiCore::detect_data_dependency(){
...
bool multiCore::detect_control_dependency(){

	return (pipeline && isBranchTaken);
}

string multiCore::disassemble (unsigned int inst_word){
...
string multiCore::registerstring(unsigned int a){
	stringstream ss;
	if (a >= 0 && a <= 13){
		ss<<"r"<<dec<<a;
	}
...
string multiCore::sintstring(unsigned int a, int size){
	a = (a<<(32-size))>>(32-size);
...
string multiCore::hexstring(unsigned int a){
	stringstream ss;
	ss<<"0x"<<hex<<a;
	return ss.str();
}

void multiCore::decTobinary(std::string &inst, unsigned int imm){
	//std::stringstream ss;
	int tmp = 0;
	//bool chk = false;
